...
export const ItemDetailModal: React.FC<ItemDetailModalProps> = ({ item, onClose, password }) => {
  ...
  const handleDownload = (fileId: string, fileName: string) => {
    // 使用 token 参数进行鉴权下载
    const downloadUrl = `/api/file/${fileId}?token=${encodeURIComponent(password)}`;
          ...
          {item.files.length > 0 && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-3">
                {item.files.map((file) => (
                  <div key={file.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 border border-gray-200 rounded-xl hover:border-black/20 hover:shadow-md transition-all group bg-white">
                    <div className="flex items-start sm:items-center space-x-3 overflow-hidden mb-3 sm:mb-0">
